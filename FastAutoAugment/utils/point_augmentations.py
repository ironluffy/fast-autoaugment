...
def random_crop_plane(point_clouds, min_rad=0.6, occlude_range=0.3):
    B, C, N = point_clouds.shape
    device = point_clouds.device
    min_rad = min_rad * (1 - occlude_range)
    cropped_point_clouds = torch.zeros([0], dtype=torch.float32, device=device)

    for idx in range(B):
        point_cloud = point_clouds[idx, :].unsqueeze(0)
        rad = np.random.uniform(min_rad, min_rad + occlude_range)
...
def random_crop_sphere(point_clouds, min_rad=0.6, occlude_range=0.3):
    B, C, N = point_clouds.shape
    device = point_clouds.device
    min_rad = min_rad * (0.8 - occlude_range)
    cropped_point_clouds = torch.zeros([0], dtype=torch.float32, device=device)
...
def random_crop_sphere_reverse(point_clouds, min_rad=0.6, occlude_range=0.3):
    B, C, N = point_clouds.shape
    device = point_clouds.device
    min_rad = min_rad * (1 - (occlude_range + 0.1)) + 0.1
    cropped_point_clouds = torch.zeros([0], dtype=torch.float32, device=device)
...
def naive_point_mix_up(point_clouds, mixup_param=1.0, random_range=0.3):
    ...
    lam = (1 - random_range) * mixup_param + torch.FloatTensor(B).uniform_(0, random_range)
    lam = lam.to(device)
...
def point_mix_up(point_clouds, mixup_param=1.0, random_range=0.3):
    emd_loss2 = emdModule()
    B, _, N = point_clouds.shape
    device = point_clouds.device
...
def squeeze_z(point_cloud, min_squeeze_param=0.6, random_range=0.3):
    B, C, N = point_cloud.shape
    device = point_cloud.device
    min_squeeze_param = min_squeeze_param * (1 - random_range)
    squeeze_param = torch.FloatTensor(B, 1).uniform_(min_squeeze_param, min_squeeze_param + random_range)

    scale_vector = torch.cat([torch.ones(point_cloud.size(0), 1),
                              torch.ones(point_cloud.size(0), 1), squeeze_param], dim=1).to(device)

    squeeze_point_cloud = point_cloud * scale_vector.unsqueeze(-1)
...
def squeeze_xy(point_cloud, min_squeeze_param=0.6, random_range=0.3):
    B, C, N = point_cloud.shape
    device = point_cloud.device
    min_squeeze_param = min_squeeze_param * (1 - random_range)
    squeeze_param = torch.FloatTensor(B, 1).uniform_(min_squeeze_param, min_squeeze_param + random_range)
...
def squeeze_sphere(point_clouds, min_squeeze_param=0.6, random_range=0.3):
    B, C, N = point_clouds.shape
    device = point_clouds.device
    min_squeeze_param = min_squeeze_param * (1 - random_range)
